...
describe('GET /routes', () => {
  it('returns all 5 seeded routes with status 200', async () => {
    const res = await request(handle.app).get('/routes');
...
describe('GET /routes/comparison', () => {
  it('returns baseline and comparison results', async () => {
    const res = await request(handle.app).get('/routes/comparison');
    expect(res.status).toBe(200);
  ...
  it('ROUTE-002 (ghg=95.5) is non-compliant', async () => {
    ...
    expect(r2.percentDiff).toBeGreaterThan(0);
  });
...
describe('POST /routes/:id/baseline', () => {
  it('sets a new baseline and returns the updated route', async () => {
    const allRes = await request(handle.app).get('/routes');
    const route3 = allRes.body.find((r: any) => r.routeId === 'ROUTE-003');

    const res = await request(handle.app).post(`/routes/${route3.id}/baseline`);
    expect(res.status).toBe(200);
  ...
  it('only one route is baseline after re-assignment', async () => {
    ...
    await request(handle.app).post(`/routes/${route1.id}/baseline`);
  });

  it('returns 404 for an unknown route id', async () => {
    const res = await request(handle.app).post('/routes/999999/baseline');
    expect(res.status).toBe(404);
  });

  it('returns 400 for a non-integer id', async () => {
    const res = await request(handle.app).post('/routes/abc/baseline');
